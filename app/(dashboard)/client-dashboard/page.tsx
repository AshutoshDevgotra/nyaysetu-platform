      ...
      <main className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-[#1a1a1a] border border-[#ffcc99] rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-[#ffcc99]">Legal AI Assistant</h2>
            <Link href="/client-dashboard/chat">
              <Button className="bg-[#ffcc99] text-black hover:bg-[#ffe0b3]">
                Open Chat Assistant
              </Button>
            </Link>
          </div>
          <p className="text-gray-400 text-sm mb-4">Get instant guidance on common legal situations.</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { title: "Traffic Challan", icon: "🚗", path: "/client-dashboard/rights?topic=challan" },
              { title: "Property Dispute", icon: "🏠", path: "/client-dashboard/rights?topic=property" },
              { title: "Domestic Violence", icon: "⚖️", path: "/client-dashboard/rights?topic=domestic" },
              { title: "Check Free Legal Aid", icon: "📋", path: "/client-dashboard/nalsa-eligibility" }
            ].map((item, i) => (
              <Link href={item.path} key={i} className="block h-full">
                <div className="bg-[#262626] border border-[#333] hover:border-[#ffcc99] transition-colors rounded-xl p-4 flex flex-col items-center text-center gap-2 cursor-pointer h-full">
