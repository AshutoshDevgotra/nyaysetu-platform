          ...
          <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            {response && (
              <div className="bg-[#1a1a1a] border border-[#ffcc99] text-white p-6 rounded-lg shadow-lg mb-8">
                ...
                <div className="text-[#ffe0b3] leading-relaxed prose prose-invert max-w-none">
                  <ReactMarkdown
                    components={{
                      p: ({ children }) => <p className="mb-3">{children}</p>,
                      ul: ({ children }) => <ul className="list-disc ml-5 mb-3 space-y-1">{children}</ul>,
                      ol: ({ children }) => <ol className="list-decimal ml-5 mb-3 space-y-1">{children}</ol>,
                      li: ({ children }) => <li className="text-[#ffe0b3]">{children}</li>,
                      strong: ({ children }) => <strong className="text-[#ffcc99] font-semibold">{children}</strong>,
                      em: ({ children }) => <em className="text-[#ffe0b3]">{children}</em>,
                      h1: ({ children }) => <h1 className="text-xl font-bold text-[#ffcc99] mb-2">{children}</h1>,
                      h2: ({ children }) => <h2 className="text-lg font-bold text-[#ffcc99] mb-2">{children}</h2>,
                      h3: ({ children }) => <h3 className="text-md font-semibold text-[#ffcc99] mb-2">{children}</h3>,
                    }}
                  >
                    {response}
                  </ReactMarkdown>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <button
                ...
                className="bg-[#1a1a1a] border border-[#ffcc99]/50 text-[#ffcc99] px-8 py-4 rounded-lg font-bold text-lg hover:bg-[#ffcc99]/10 transition-all duration-300 flex items-center justify-center gap-2"
              >
                <span>Join Waitlist</span>
                <span className="text-xs bg-[#ffcc99] text-black px-2 py-0.5 rounded-full">New</span>
              </button>
            </div>
          </div>
        </div>
